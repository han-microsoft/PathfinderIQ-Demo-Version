...
Module role:
    ...
    When ``OTEL_EXPORT_TARGET=""`` (noop mode), ``trace_id`` and ``span_id``
...
class _CorrelationFilter(logging.Filter):
    ...
    The ``request_id`` is set by ``_middleware.correlation_dispatch()`` at
    the start of each HTTP request.  Log records emitted outside a request
    context (e.g., during startup) get an empty string — harmless in JSON.
...
def configure_json_logging() -> None:
    ...
    Reads the ``DEBUG`` env var (or ``settings.debug``) to set the root log
    level.  Clears any existing handlers (including ``basicConfig``'s default
    ``StreamHandler``) and installs a single JSON-formatted ``StreamHandler``
    writing to stdout.
