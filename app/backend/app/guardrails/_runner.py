...
Module role:
    Runs a list of guardrails in order. First BLOCK stops the chain.
    WARN results are logged but don't stop. Exceptions from individual
    guardrails are caught and logged — never block the request (fail-open).

Key collaborators:
    - ``app.guardrails`` — GuardrailResult, GuardrailVerdict protocols
    - ``app.routers.chat`` — calls execute_input_guardrails()

Dependents:
    Called by: chat.py (HTTP layer), future AgentGuardrailMiddleware.
"""
