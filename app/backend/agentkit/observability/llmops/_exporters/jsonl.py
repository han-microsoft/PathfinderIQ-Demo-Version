...
Simplest ``TraceExporter``. One JSON object per line. Uses ``asyncio.to_thread``
for non-blocking writes (no ``aiofiles`` dependency). Each export opens,
appends, and closes — no persistent file handle. Size-based rotation at
10 MB prevents disk exhaustion on containers with limited ephemeral storage.
...
from agentkit.observability.llmops._protocol import LLMTrace
