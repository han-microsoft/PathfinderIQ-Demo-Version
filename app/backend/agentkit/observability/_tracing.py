...
Module role:
    ...
    5. All of this is noop when OTel export is disabled — the ``NoopTracer``
       produces zero-cost spans that discard all data.
...
Layer rule:
    stdlib + optional ``opentelemetry`` + ``agentkit.observability._metrics``.
    Domain-blind — ``name`` / ``backend`` are caller-supplied labels.
        ...
        async def wrapper(*args, **kwargs):
            ...
            with tracer.start_as_current_span(
                ...
                try:
                    ...
                    get_tool_call_counter().add(1, {"tool.name": name})
                    get_tool_duration_histogram().record(
                        elapsed_ms,
                        {"tool.name": name},
                    )
                    ...
                    get_tool_error_counter().add(1, {"tool.name": name})
                    raise
