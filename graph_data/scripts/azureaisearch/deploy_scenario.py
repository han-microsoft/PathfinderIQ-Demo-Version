...
# Resolve project root: scripts/azureaisearch/ → 2 parents up = graph_data/
...
def main() -> int:
    ...
    # Import and call the colocated provisioner in-process so env vars loaded
    # above remain available without any subprocess shell gymnastics.
    try:
        from .provision_search_index import _build_configs_from_manifest, run
    except ImportError:
        azure_search_dir = str(Path(__file__).resolve().parent)
        if azure_search_dir not in sys.path:
            sys.path.insert(0, azure_search_dir)
        from provision_search_index import _build_configs_from_manifest, run
