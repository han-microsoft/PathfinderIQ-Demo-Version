...
Usage:
    ...
    uv run python3 scripts/azureaisearch/provision_search_index.py --manifest data/scenarios/airline-ops/search_manifest.yaml

    # With file upload:
    uv run python3 scripts/azureaisearch/provision_search_index.py --manifest data/scenarios/airline-ops/search_manifest.yaml --upload-files

    # Dry run (preview only):
    uv run python3 scripts/azureaisearch/provision_search_index.py --manifest data/scenarios/airline-ops/search_manifest.yaml --dry-run

    # Legacy mode (backward compat — reads from scenario_loader):
    DEFAULT_SCENARIO=telecom-playground uv run python3 scripts/azureaisearch/provision_search_index.py --upload-files
...
def _create_indexer(
    ...
    # Retry indexer creation — RBAC propagation can take 1–5 minutes after
    # role assignment. The 10s sleep after assignment is insufficient.
    for _rbac_attempt in range(12):
        try:
            indexer_client.create_or_update_indexer(indexer)
            break
        except Exception as e:
            if "403" in str(e) and _rbac_attempt < 11:
                print(f"    ⚠ RBAC not propagated yet, waiting 15s ({_rbac_attempt + 1}/12)")
                time.sleep(15)
            else:
                raise
